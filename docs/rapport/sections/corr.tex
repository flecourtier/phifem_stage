...
\begin{equation*}
	\tilde{\phi}=u_{MLP}
\end{equation*}
with $u_{MLP}$ the MLP predicition.
...
Here we choose to train the network with $n_{pts}$ points randomly selected in the domain, and consider for each epoch a new training sample defined by
\begin{align*}
	X\_train&=\{(x_i,y_i)\}_{i=1,\dots,n_{pts}} \\
	Y\_train&=\{f_i\}_{i=1,\dots,n_{pts}} 
\end{align*}
with $(x_i,y_i)$ the coordinates of the points and $f_i=f(x_i,y_i)$ the second member of the problem evaluated at each points.
...
\begin{equation*}
	\tilde{\phi}=u_{PINNS}
\end{equation*}
with $u_{PINNS}$ the PINNS predicition.

To do this, we consider $\tilde{\phi}$ in $\mathbb{P}_10$ for $n_{vert}=32$.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\linewidth]{corr_networks/PINNs_FEM_add.png}
...
We obtain an error $||u_{ex}-\tilde{\phi}C||_{0,\Omega}^{(rel)}=2.98e-5$. Thus, it would seem that the results are very promising: for this problem, the FEM error is $2.80e-2$, so we obtain an error reduction of $940.89$.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\linewidth]{corr_networks/PINNs_PhiFEM_add.png}
...
We obtain an error $||u_{ex}\tilde{\phi}C||_{0,\Omega}^{(rel)}=8.92e-4$. Thus, it would seem that the results are not very promising with $\phi$-FEM: for this problem, the $\phi$-FEM error is $1.92e-2$, so we only obtain an error reduction of $21.61$.

Consequently, we're going to look too at the first (Figure \ref{corr_networks_PINNs_derivees_premieres}) and second (Figure \ref{corr_networks_PINNs_derivees_secondes}) derivatives of the PINNs prediction, which will be calculated with Pytorch and FEniCS and compared with the exact derivatives. 
...
\begin{Rem}
	The derivatives displayed on the left in Figure \ref{corr_networks_PINNs_derivees_premieres} and Figure \ref{corr_networks_PINNs_derivees_secondes} are on the $\mathcal{O}$ domain, so a mask must be applied and the derivatives on the right which are displayed on the $\Omega$ domain.
\end{Rem}

\begin{Rem}
	We found an error in the PINNs part that could not be resolved in time to put the results in this report. In fact, we trained the model on the $[0,1]^2$ domain, which is our $\Omega$ domain, but for $\phi$-FEM we had to consider a surrounding domain to $\Omega$. We therefore need to train PINNs on $\mathcal{0}=[-0.5,1.5]^2$, which would explain why the correction with FEM works but the one with $\phi$-FEM doesn't work properly.
\end{Rem}