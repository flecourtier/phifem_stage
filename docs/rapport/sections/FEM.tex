...
\modif{Attention là cas homogène !}
...
\end{equation*}
Given an approximation $\phi_h$ of $\phi$ on the mesh $\mathcal{T}_h$, as defined in Section \ref{FEM.phifem.description_spaces}, and a finite element space $V_h$ on $\mathcal{T}_h$, we can then search for $w_h\in V_h$ such that
\begin{equation*}
...
\modif{Ajouter remarque en disant que $\Omega_h$ dépend implicitement de $\phi$ ?}

\modif{Ajouter remarque terme au bord ne s'annule pas sur $\partial\Omega_h$ + expliquer ce que c'est crochets - termes de saut.}

\modif{AJOUTER Explications termes de stabilisation !}

\modif{Ajouter explication cas non homogène (dual et direct).}