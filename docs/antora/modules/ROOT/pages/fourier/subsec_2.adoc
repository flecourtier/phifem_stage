:stem: latexmath
:xrefstyle: short
= Some details on the convolution sub-layer
...
In reality, we'll be using a specific implementation of FFT, called RFFT (Real Fast Fourier Transform). In fact, for stem:[\mathcal{F}^{-1}(A)] to be real if stem:[A] is a complex-valued matrix, it is necessary that A respects the Hermitian symmetry:
[stem]
++++
